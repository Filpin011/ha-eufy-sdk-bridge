...
| `SNAPSHOT_LIVE` | `auto` | how `/snapshot/<sn>` (the still HA shows on a camera tile) gets its picture. `auto`: a **mains** camera gets a fresh live burst; a **battery** camera answers from its last-event thumbnail and is not woken, because HA re-fetches stills on a timer and each live burst is a radio wake. `1` forces the live burst for every camera, `0` never takes one. When no live still is taken or it fails, the persisted last-event thumbnail is served instead of an error |
